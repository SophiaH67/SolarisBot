...
namespace SolarisBot.Discord.Modules.Reminders
{
    ...
    internal sealed class ReminderService : IHostedService
    {
        ...
        private async Task RemindUsersAsync()
        {
            ...
            var reminders = await dbCtx.Reminders.FromSqlRaw($"SELECT * FROM reminders WHERE time <= {nowUnix} AND deletedat is not null").ToArrayAsync(); //UInt equality not supported
