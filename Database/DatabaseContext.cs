            ...
            try
            {
                ...
                if (version < 1)
                {
                    queries.AddRange(new List<string>() //todo: [FEATURE] Block deletion of guild configs?
                    {
                        ...
                        "CREATE TRIGGER RoleGroupsAvoidDuplicateInsert BEFORE INSERT ON RoleGroups FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate role group on insert') WHERE EXISTS (SELECT 1 FROM RoleGroups WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.Identifier = Identifier)); END;",
                        "CREATE TRIGGER RoleGroupsAvoidDuplicateUpdate BEFORE UPDATE ON RoleGroups FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.RoleGroupId != OLD.RoleGroupId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate role group on update') WHERE EXISTS (SELECT 1 FROM RoleGroups WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.Identifier = Identifier)); END;",
                        "CREATE TRIGGER RoleGroupsSoftDelete BEFORE DELETE ON RoleGroups FOR EACH ROW BEGIN UPDATE RoleGroups SET DeletedAt = strftime('%s', 'now') WHERE RoleGroupId = OLD.RoleGroupId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER RoleConfigsAvoidDuplicateInsert BEFORE INSERT ON RoleConfigs FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate role config on insert') WHERE EXISTS (SELECT 1 FROM RoleConfigs WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND (NEW.RoleId = RoleId OR (NEW.RoleGroupId = RoleGroupId AND NEW.Identifier = Identifier)))); END;",
                        "CREATE TRIGGER RoleConfigsAvoidDuplicateUpdate BEFORE UPDATE ON RoleConfigs FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.RoleConfigId != OLD.RoleConfigId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate role config on update') WHERE EXISTS (SELECT 1 FROM RoleConfigs WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND (NEW.RoleId = RoleId OR (NEW.RoleGroupId = RoleGroupId AND NEW.Identifier = Identifier)))); END;",
                        "CREATE TRIGGER RoleConfigsSoftDelete BEFORE DELETE ON RoleConfigs FOR EACH ROW BEGIN UPDATE RoleConfigs SET DeletedAt = strftime('%s', 'now') WHERE RoleConfigId = OLD.RoleConfigId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER QuotesAvoidDuplicateInsert BEFORE INSERT ON Quotes FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate quote on insert') WHERE EXISTS (SELECT 1 FROM Quotes WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND (NEW.MessageId = MessageId OR (NEW.AuthorId = AuthorId AND NEW.GuildId = GuildId AND NEW.Text = Text)))); END;",
                        "CREATE TRIGGER QuotesAvoidDuplicateUpdate BEFORE UPDATE ON Quotes FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.QuoteId != OLD.QuoteId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate quote on update') WHERE EXISTS (SELECT 1 FROM Quotes WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND (NEW.MessageId = MessageId OR (NEW.AuthorId = AuthorId AND NEW.GuildId = GuildId AND NEW.Text = Text)))); END;",
                        "CREATE TRIGGER QuotesSoftDelete BEFORE DELETE ON Quotes FOR EACH ROW BEGIN UPDATE Quotes SET DeletedAt = strftime('%s', 'now') WHERE QuoteId = OLD.QuoteId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER JokeTimeoutsAvoidDuplicateInsert BEFORE INSERT ON JokeTimeouts FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate joke timeout on insert') WHERE EXISTS (SELECT 1 FROM JokeTimeouts WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.UserId = UserId)); END;",
                        "CREATE TRIGGER JokeTimeoutsAvoidDuplicateUpdate BEFORE UPDATE ON JokeTimeouts FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.JokeTimeoutId != OLD.JokeTimeoutId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate joke timeout on update') WHERE EXISTS (SELECT 1 FROM JokeTimeouts WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.UserId = UserId)); END;",
                        "CREATE TRIGGER JokeTimeoutsSoftDelete BEFORE DELETE ON JokeTimeouts FOR EACH ROW BEGIN UPDATE JokeTimeouts SET DeletedAt = strftime('%s', 'now') WHERE JokeTimeoutId = OLD.JokeTimeoutId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER RemindersAvoidDuplicateInsert BEFORE INSERT ON Reminders FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate reminder on insert') WHERE EXISTS (SELECT 1 FROM Reminders WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.UserId = UserId AND NEW.Text = Text)); END;",
                        "CREATE TRIGGER RemindersAvoidDuplicateUpdate BEFORE UPDATE ON Reminders FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.ReminderId != OLD.ReminderId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate reminder on update') WHERE EXISTS (SELECT 1 FROM Reminders WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.GuildId = GuildId AND NEW.UserId = UserId AND NEW.Text = Text)); END;",
                        "CREATE TRIGGER RemindersSoftDelete BEFORE DELETE ON Reminders FOR EACH ROW BEGIN UPDATE Reminders SET DeletedAt = strftime('%s', 'now') WHERE ReminderId = OLD.ReminderId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER BridgesAvoidDuplicateInsert BEFORE INSERT ON Bridges FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate bridge on insert') WHERE EXISTS (SELECT 1 FROM Bridges WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND ((ChannelAId = NEW.ChannelAId AND ChannelBId = NEW.ChannelBId) OR (ChannelBId = NEW.ChannelAId AND ChannelAId = NEW.ChannelBId)))); END;",
                        "CREATE TRIGGER BridgesAvoidDuplicateUpdate BEFORE UPDATE ON Bridges FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.BridgeId != OLD.BridgeId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate bridge on update') WHERE EXISTS (SELECT 1 FROM Bridges WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND ((ChannelAId = NEW.ChannelAId AND ChannelBId = NEW.ChannelBId) OR (ChannelBId = NEW.ChannelAId AND ChannelAId = NEW.ChannelBId)))); END;",
                        "CREATE TRIGGER BridgesSoftDelete BEFORE DELETE ON Bridges FOR EACH ROW BEGIN UPDATE Bridges SET DeletedAt = strftime('%s', 'now') WHERE BridgeId = OLD.BridgeId; SELECT RAISE(IGNORE); END;",
                        ...
                        "CREATE TRIGGER RegexChannelsAvoidDuplicateInsert BEFORE INSERT ON RegexChannels FOR EACH ROW WHEN NEW.DeletedAt IS NULL BEGIN SELECT RAISE(ABORT, 'Duplicate regex channel on insert') WHERE EXISTS (SELECT 1 FROM RegexChannels WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.ChannelId = ChannelId)); END;",
                        "CREATE TRIGGER RegexChannelsAvoidDuplicateUpdate BEFORE UPDATE ON RegexChannels FOR EACH ROW WHEN NEW.DeletedAt IS NULL AND (NEW.RegexChannelId != OLD.RegexChannelId OR NEW.DeletedAt != OLD.DeletedAt) BEGIN SELECT RAISE(ABORT, 'Duplicate regex channel on update') WHERE EXISTS (SELECT 1 FROM RegexChannels WHERE (NEW.DeletedAt IS NULL AND DeletedAt IS NULL AND NEW.ChannelId = ChannelId)); END;",
                        "CREATE TRIGGER RegexChannelsSoftDelete BEFORE DELETE ON RegexChannels FOR EACH ROW BEGIN UPDATE RegexChannels SET DeletedAt = strftime('%s', 'now') WHERE RegexChannelId = OLD.RegexChannelId; SELECT RAISE(IGNORE); END;",
